...
func (s *Store) Claim(ctx context.Context) (*model.Commit, error) {
	...
	err = tx.QueryRowContext(ctx, `
		...
		LIMIT 1`).Scan(&commit.ID, &userID, &commit.ExerciseID, &created, &s3Key)
	...
	if userID.Valid {
		commit.UserID = userID.Int64
	}
	if created.Valid {
		commit.CreatedAt = created.Time
	}

	// Mark the commit as `compiling` so other workers skip it.
	if _, err := tx.ExecContext(ctx, `
		UPDATE commits SET status = 'compiling', compilation_started = now()
		WHERE id = $1`, commit.ID); err != nil {
		return nil, fmt.Errorf("claim commit %d: %w", commit.ID, err)
	}
...
func (s *Store) FetchTestCases(ctx context.Context, exerciseID int64) ([]model.TestCase, error) {
	// Fetch the test cases first, then fetch the files in a second query to avoid
	// the N+1 problem. We use a map to associate test case IDs with their files.
	...
	// Scan the test cases into a slice and collect their IDs for the next query.
	var cases []model.TestCase
	var ids []int64

	for rows.Next() {
		// Scan each test case row into a TestCase struct.
	...
	// Fetch the files for the test cases in a single query using the collected IDs.
	...
	// Map test case IDs to their associated file paths.
	...
	// Associate the fetched files with their corresponding test cases.
	for i := range cases {
...
func (s *Store) FetchCompilationFiles(ctx context.Context, exerciseID int64) ([]string, error) {
	// Fetch the compilation files for the exercise, ordered by their ID.
	rows, err := s.db.QueryContext(ctx, `
	...
	// Scan the paths into a slice of strings.
	var paths []string
